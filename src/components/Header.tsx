...
export default () => {
  ...
  return (
    <>
      ...
      <menu data-state={menuOpened ? "open" : "closed"} style={{ pointerEvents: menuOpened ? "unset" : "none" }} className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fill-mode-forwards">
        {/* bg */}
        ...
        <div data-state={menuOpened ? "open" : "closed"} className="fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 inset-y-0 left-0 h-full w-3/4 border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-sm pr-0 fill-mode-forwards">
          ...
          {/* close menu btn */}
          <button onClick={() => setMenuOpened(false)} data-state={menuOpened ? "open" : "closed"} className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
